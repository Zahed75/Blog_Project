...
def user_change(request):
    ...
    if request.method == 'POST':
        form = UserProfileChange(request.POST,instance=current_user) # value replace kore value update korbe

        if form.is_valid():
            form.save()
            form = UserProfileChange(instance=current_user) #updated value reload korbe
